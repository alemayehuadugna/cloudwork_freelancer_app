...
import '../repo/bid_repository.dart';

class ListBid implements UseCase<List<JobEntity>, PaginationParams> {
  final BidRepository repository;

  ListBid({required this.repository});
  ...
  Future<Either<Failure, List<JobEntity>>> call(PaginationParams params) async {
    ...
    Either<Failure, List<JobEntity>> jobs = await repository.listBids(params);
    jobs.fold((failure) {
      return Left(failure);
    }, (jobs) {
      jobs.forEach(
